    ...
    def _predict_single(self, image):
        ...
        if not self.multiperson:
            ...
            # if detections and len(detections) > 0 and detections[0].boxes and len(detections[0].boxes) > 0 and detections[0].boxes[0].data:
                # print(detections)
                for i, (x1, y1, x2, y2, cls, cls_conf, cls_pred) in enumerate(detections):
            ...
            # if detections and len(detections) > 0 and detections[0].boxes and len(detections[0].boxes) > 0 and detections[0].boxes[0].data:
                # print(detections)
                for i, (x1, y1, x2, y2, cls_conf, cls_pred) in enumerate(detections[0].boxes[0].data):
                # for i, detection in enumerate(detections):
                    x1 = int(round(x1.item()))
                    x2 = int(round(x2.item()))
                    y1 = int(round(y1.item()))
    ...
    def _predict_batch(self, images):
        ...
        if not self.multiperson:
            ...
            for d, detections in enumerate(image_detections):
                image = images[d]
                if detections is not None and len(detections) > 0 and self.yolo_version != 'v8':
                    for i, (x1, y1, x2, y2, cls, cls_conf, cls_pred) in enumerate(detections):
                        x1 = int(round(x1.item()))
                        x2 = int(round(x2.item()))
                        y1 = int(round(y1.item()))
