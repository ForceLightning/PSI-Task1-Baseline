...
def video():
    ...
    for index, images_dir in tqdm(enumerate(input_image_save_dirs)):
        ...
        for image_path in tqdm(image_list):
            ...
            if not candidate_bbox:
                video_candidates_list.append({"image_path": image_path, "bbox": None, "keypoints": None})
            else:
                for bbox in candidate_bbox:
                    video_candidates_list.append({"image_path": image_path, "bbox": bbox, "keypoints": None})
        video_candidates[video_name] = {"candidates_list": video_candidates_list, "length": len(image_list)}
    logger.info("Person Instance detection finish")

    # 3. Single Person Pose Estimation
    ...
    for video_name, video_info in video_candidates.items():
        ...
        for person_info in tqdm(video_candidates_list):
            ...
            if xywh_box:
                keypoints = inference_PE(image_path, prev_image_path, next_image_path, xywh_box)
            ...
            if SAVE_VIS_IMAGE:
                image_save_path = os.path.join(base_img_vis_save_dirs, video_name, os.path.basename(image_path))
                ...
                if xywh_box:
                    ...
                    pose_img = current_image
                save_image(image_save_path, pose_img)

            if SAVE_BOX_IMAGE:
                image_save_path = os.path.join(base_img_vis_box_save_dirs, video_name, os.path.basename(image_path))
                if osp.exists(image_save_path):
                    current_image = read_image(image_save_path)
                else:
                    current_image = read_image(image_path)
                if xywh_box:
                    xyxy_box = xywh_box[0], xywh_box[1], xywh_box[0] + xywh_box[2], xywh_box[1] + xywh_box[3]
                    box_image = add_bbox_in_image(current_image, xyxy_box)
                else:
                    box_image = current_image
