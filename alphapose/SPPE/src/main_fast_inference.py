...
class InferenNet(nn.Module):
    def __init__(self, kernel_size, dataset):
        ...
        # print('Loading pose model from {}'.format('./models/halpe/halpe26_fast_res50_256x192.pth'))
        sys.stdout.flush()
        model.load_state_dict(torch.load('./models/sppe/duc_se.pth'))
        # model.load_state_dict(torch.load('./models/halpe/halpe26_fast_res50_256x192.pth'))
        model.eval()
        self.pyranet = model
...
class InferenNet_fast(nn.Module):
    def __init__(self, kernel_size, dataset):
        ...
        model = createModel().cuda()
        print('Loading pose model from {}'.format('alphapose/SPPE/src/models/sppe/duc_se.pth'))
        model.load_state_dict(torch.load('alphapose/SPPE/src/models/sppe/duc_se.pth'))
        # print('Loading pose model from {}'.format('./models/halpe/halpe26_fast_res50_256x192.pth'))
        # model.load_state_dict(torch.load('./models/halpe/halpe26_fast_res50_256x192.pth'))
        model.eval()
        self.pyranet = model
